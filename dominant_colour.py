...
def get_dominant_colour_from_image(image_path):
    ...
    # dominant_color = color_thief.get_palette(color_count=2, quality=1)[0]
...
class PictureRejigger:
    ...
    def __init__(
        ...
        if self.input_pixels.endswith("json"):
            self.pixel_rgb = self.convert_keys_from_json_dump()
        else:
    ...
    def get_colour_difference(self, pixel_rbg, image_rbg):
        ...
        r_distance = math.pow((pr - ir), 2)
        g_distance = math.pow((pg - ig), 2)
        b_distance = math.pow((pb - ib), 2)

        self.logger.debug(
            f"r_dist={r_distance}, g_dist={g_distance}, b_dist={b_distance}"
        )

        rgb_rms_distance = math.sqrt((r_distance + g_distance + b_distance) / 3)
    ...
    def get_rgb_dict_from_new_pixels(self):
        ...
        picture_dict = defaultdict(dict)
        for image in os.listdir(self.input_pixels):
            select_image_path = os.path.join(self.input_pixels, image)

            # 4 number returned: Alpha (transparency) + RGB
            picture_dict[select_image_path] = {
                "rgb": get_dominant_colour_from_image(select_image_path),
    ...
    def create_new_blank_background(self, input_image_for_size):
        ...
        new_img = Image.new("RGBA", (new_x, new_y), (1, 0, 0, 0),)
    ...
    def run_process(self):
        ...
        image_x, image_y = self.blurred_image.size
        range_x = range(image_x)
        range_y = range(image_y)
        ...
        img_parse_dict = self.alter_dictionary_for_json_dump()
        with open(self.input_pixels, "w") as handle:
            json.dump(img_parse_dict, handle)

    def alter_dictionary_for_json_dump(self):
        """
        can't dump a dictionary with tuple keys to a json file - reformat the keys first
        :return:
        """
        new_dict = dict()
        for file_key in self.pixel_rgb:
            new_dict[file_key] = {
                "rgb": self.pixel_rgb[file_key]["rgb"],
                "checked": dict(),
            }

            for each_check in self.pixel_rgb[file_key]["checked"].keys():
                new_key = "-".join(map(str, each_check))

                new_dict[file_key]["checked"][new_key] = self.pixel_rgb[file_key][
                    "checked"
                ][each_check]

        return new_dict

    def convert_keys_from_json_dump(self):
        """
        I'd like a tuple as keys, but tuple keys in a dictionary can't be dumped to JSON
        :return:
        """

        json_from_file = json.load(open(self.input_pixels))
        new_dict = dict()
        for file_key in json_from_file:
            new_dict[file_key] = {
                "rgb": json_from_file[file_key]["rgb"],
                "checked": dict(),
            }

            for each_check in json_from_file[file_key]["checked"].keys():
                new_key = (x for x in each_check.split("-"))

                new_dict[file_key]["checked"][new_key] = json_from_file[file_key][
                    "checked"
                ][each_check]
    ...
    def compare_pixel_to_input_images(self, input_rgb):
        ...
        for image_path in self.pixel_rgb:

            if not input_rgb in self.pixel_rgb[image_path]["checked"]:
                difference = self.get_colour_difference(
                    input_rgb, self.pixel_rgb[image_path]["rgb"]
                )
                self.logger.debug(f"calculated difference: {difference}")
                self.pixel_rgb[image_path]["checked"][input_rgb] = difference

            else:
                self.logger.debug("No re-checking required!")

        # intially just assume there are no ties - find the best fitting key
        best_image = min(
            self.pixel_rgb.keys(),
            key=(lambda k: self.pixel_rgb[k]["checked"][input_rgb]),
        )
        return best_image


"""
multiple possible minimums in a list
>>> a = [2,4,5,2]
>>> min_value = min(a)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "or the JSON dump of previous JSON run",
    )
    parser.add_argument(
